...
u32 crc32_halfbyte(u32 crc, const void *buf, size_t len)
{
	...
	while (len--) {
		crc = crc_table_le4[(crc ^  *data) & 0x0F] ^ (crc >> 4);
		crc = crc_table_le4[(crc ^ (*data >> 4)) & 0x0F] ^ (crc >> 4);
		data++;
	}
 ...
 * @buf: pointer to buffer over which CRC32/CRC32C is run
 * @len: length of buffer @buf
 * @tab: little-endian Ethernet table
 * @poly: CRC32/CRC32c LE poly
 */
...
u32 crc32(u32 crc, const void *buf, size_t len)
{
	...
	while (len--) {
		crc = crc_table_le8[(crc ^ *data++) & 0xFFL] ^ (crc >> 8);
	}
        ...
        u32 crc, const void *buf, size_t len, u32 poly)
{
	...
	while (len--) {
		crc ^= *data++;
		crc = (crc >> 4) ^ crc_table_le4[crc & 0xf];
		crc = (crc >> 4) ^ crc_table_le4[crc & 0xf];
	}
#elif (CRC_LE_BITS == 8)
...
#else
#error "error: wrong CRC_LE_BITS setting"
#endif
